...
window.addEventListener('DOMContentLoaded', () => {
    // PROTEKSI: Tendang ke root luar jika tidak ada sesi login sama sekali
    if (!SESI_WARGA) { window.location.href = "../"; return; }
    
    // Atur Tampilan Menu berdasarkan Tipe Akun (Anggota Tetap / PON)
...
function inisialisasiSistemTampilanWarga(dataUser) {
    ...
    const badgePortal = document.getElementById('txtPortalBadge');
    const tombolNavLog = document.getElementById('nav-btn-log');

    if (tipeAkun === 'PON') {
        if (badgePortal) badgePortal.innerText = "Portal Warga PON";
        if (tombolNavLog) tombolNavLog.classList.add('hidden'); // Sembunyikan tab log sampah untuk PON
    } else {
        if (badgePortal) badgePortal.innerText = "Portal Anggota Tetap";
...
function switchTab(id) {
    if (id === 'scr-log' && (SESI_WARGA.tipe || '').toUpperCase() === 'PON') { id = 'scr-home'; }
    ...
    try {
        ...
        Object.keys(data).forEach(key => {
            ...
            if(i.warga_key === SESI_WARGA.key) {
                ...
                // Semua diarahkan ke kuitansi.html tunggal memakai token_kuitansi atau firebase key
                const urlKuitansi = `../kuitansi.html?id=${i.token_kuitansi || key}`;
                const labelSubHeader = (i.tipe_user === "PON") ? `BAKAR SAMPAH (${i.hari || 'Harian'})` : i.bulan;
                    ...
                    <div class="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm space-y-3">
                        <div class="flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                ...
                                <div><h4 class="text-xs font-extrabold text-slate-800 uppercase">${labelSubHeader}</h4><p class="text-[9px] text-slate-400 font-medium">${i.tanggal}</p></div>
                            </div>
...
function generateKalenderSampah() {
    ...
    const grid = document.getElementById('gridHariKalender'); if (!grid) return; grid.innerHTML = "";
    ...
    const tanggalPertama = new Date(tahun, bulan - 1, 1); const tanggalTerakhir = new Date(tahun, bulan, 0);
    
    for (let i = 0; i < tanggalPertama.getDay(); i++) { grid.insertAdjacentHTML('beforeend', `<div></div>`); }
...
async function muatSistemLogSampah() {
    try {
        ...
        DATA_LOG_SAMPAH_GLOBAL = await res.json() || {}; generateKalenderSampah();
    } catch(e){}
}
...
function logoutSession() { localStorage.clear(); window.location.href = '../'; }
