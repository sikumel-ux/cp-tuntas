...
window.addEventListener('DOMContentLoaded', () => {
    // PROTEKSI: Tendang ke root luar HANYA jika tidak ada sesi login sama sekali
    if (!SESI_WARGA) { window.location.href = "../"; return; }
    
    // Atur Tampilan berdasarkan Tipe Akun (Anggota Tetap / PON)
...
// FUNGSI SWITCH ROLE PORTAL & NAVIGASI
...
async function sinkronUlangData() {
    ...
    try { 
        // Warga PON tidak perlu memuat sistem log sampah kalender
...
function switchTab(id) {
    // Amankan jika warga PON mencoba memicu tab log lewat konsol/inspeksi
...
// =========================================================================
// FIX TOTAL: SATU JALUR LINK KUITANSI DIGITAL UNTUK TETAP & NON-TETAP (PON)
// =========================================================================
    ...
    try {
        ...
        Object.keys(data).forEach(key => {
            ...
            if(i.warga_key === SESI_WARGA.key) {
                ...
                // Menyatukan target ke satu file kuitansi.html tunggal sesuai kesepakatan
                // Mengambil token_kuitansi atau fallback ke field 'pon' jika akun bertipe PON
                const tokenID = i.token_kuitansi || i.pon || key;
...
function generateKalenderSampah() {
    // Abaikan pembentukan kalender jika user adalah PON
